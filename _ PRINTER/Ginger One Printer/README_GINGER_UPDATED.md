...
## 🔧 Extended Pre-Print Setup

Before starting the print, make sure the following steps are carried out:
...
### Heating System
- **Extruder Heating**: On the **right-hand display**, set the target temperature using the rotary knob or touchscreen. Activate the heating system and wait until the **target temperature is reached** before moving forward.

---

## ▶️ Extended Print Process Overview

- **Insert Print File**: Export your sliced G-code using **Larger Slicer** and place the `.gcode` file on a **microSD card**. Insert it into the **rear-left slot of the controller box**.
- **Startup Sequence**:
  1. **Home** the printer.
...
- **Z-Offset & Flow Adjustments**: Can be changed during the print via the **Tune menu** on the printer. However, ideally these should be adjusted **in the Grasshopper script** before export.

> ℹ️ Always wait for the bed to cool before attempting to remove a finished print to avoid damaging the surface or deforming the part.

---

## 🧪 Extended Material Handling & Settings
